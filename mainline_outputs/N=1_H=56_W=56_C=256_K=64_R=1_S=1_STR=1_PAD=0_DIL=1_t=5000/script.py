        ...
        for n_0_h_0_w_0_co_0_fused in T.thread_binding(196, thread="blockIdx.x", annotations={"pragma_auto_unroll_max_step":1024, "pragma_unroll_explicit":1}):
            for n_1_h_1_w_1_co_1_fused in T.thread_binding(1, thread="vthread.x"):
                for n_2_h_2_w_2_co_2_fused in T.thread_binding(64, thread="threadIdx.x"):
                    for n_3_init, h_3_init, w_3_init, co_3_init, n_4_init, h_4_init, w_4_init, co_4_init in T.grid(1, 1, 1, 2, 1, 4, 1, 2):
                        with T.block("conv2d_nhwc_init"):
                            v_n = T.axis.spatial(1, n_3_init + n_4_init)
                            v_h = T.axis.spatial(56, n_0_h_0_w_0_co_0_fused // 28 * 8 + n_2_h_2_w_2_co_2_fused // 32 * 4 + h_3_init * 4 + h_4_init)
                            v_w = T.axis.spatial(56, w_4_init + n_0_h_0_w_0_co_0_fused % 28 * 2 + n_2_h_2_w_2_co_2_fused % 32 // 16 + w_3_init)
                            v_co = T.axis.spatial(64, n_2_h_2_w_2_co_2_fused % 16 * 4 + co_3_init * 2 + co_4_init)
                            T.reads()
                            ...
                            for ax0_ax1_ax2_ax3_fused_1 in T.thread_binding(64, thread="threadIdx.x"):
                                for ax0_ax1_ax2_ax3_fused_2 in T.vectorized(2):
                                    with T.block("PadInput_shared"):
                                        ...
                                        v1 = T.axis.spatial(56, n_0_h_0_w_0_co_0_fused // 28 * 8 + (ax0_ax1_ax2_ax3_fused_0 * 128 + ax0_ax1_ax2_ax3_fused_1 * 2 + ax0_ax1_ax2_ax3_fused_2) // 128)
                                        v2 = T.axis.spatial(56, n_0_h_0_w_0_co_0_fused % 28 * 2 + (ax0_ax1_ax2_ax3_fused_0 * 128 + ax0_ax1_ax2_ax3_fused_1 * 2 + ax0_ax1_ax2_ax3_fused_2) % 128 // 64)
                                        v3 = T.axis.spatial(256, rc_0 * 64 + (ax0_ax1_ax2_ax3_fused_0 * 128 + ax0_ax1_ax2_ax3_fused_1 * 2 + ax0_ax1_ax2_ax3_fused_2) % 64)
                            ...
                            for ax0_ax1_ax2_ax3_fused_1 in T.thread_binding(64, thread="threadIdx.x"):
                                for ax0_ax1_ax2_ax3_fused_2 in T.vectorized(4):
                                    with T.block("weight_shared"):
                                        ...
                                        v2 = T.axis.spatial(256, rc_0 * 64 + (ax0_ax1_ax2_ax3_fused_0 * 256 + ax0_ax1_ax2_ax3_fused_1 * 4 + ax0_ax1_ax2_ax3_fused_2) // 64)
                                        v3 = T.axis.spatial(64, (ax0_ax1_ax2_ax3_fused_0 * 256 + ax0_ax1_ax2_ax3_fused_1 * 4 + ax0_ax1_ax2_ax3_fused_2) % 64)
                                        T.reads(weight[v0, v1, v2, v3])
                                        T.writes(weight_shared[v0, v1, v2, v3])
                                        weight_shared[v0, v1, v2, v3] = weight[v0, v1, v2, v3]
                        for rh_1, rw_1, rc_1, n_3, h_3, w_3, co_3, rh_2, rw_2, rc_2, n_4, h_4, w_4, co_4 in T.grid(1, 1, 4, 1, 1, 1, 2, 1, 1, 16, 1, 4, 1, 2):
                            with T.block("conv2d_nhwc_update"):
                                v_n = T.axis.spatial(1, n_3 + n_4)
                                v_h = T.axis.spatial(56, n_0_h_0_w_0_co_0_fused // 28 * 8 + n_2_h_2_w_2_co_2_fused // 32 * 4 + h_3 * 4 + h_4)
                                v_w = T.axis.spatial(56, w_4 + n_0_h_0_w_0_co_0_fused % 28 * 2 + n_2_h_2_w_2_co_2_fused % 32 // 16 + w_3)
                                v_co = T.axis.spatial(64, n_2_h_2_w_2_co_2_fused % 16 * 4 + co_3 * 2 + co_4)
                                v_rh = T.axis.reduce(1, rh_0 + rh_1 + rh_2)
                                v_rw = T.axis.reduce(1, rw_2 + rw_0 + rw_1)
                                v_rc = T.axis.reduce(256, rc_0 * 64 + rc_1 * 16 + rc_2)
                    ...
                    for ax0, ax1, ax2, ax3 in T.grid(1, 4, 1, 4):
                        with T.block("conv2d_nhwc_local"):
                            ...
                            v1 = T.axis.spatial(56, n_0_h_0_w_0_co_0_fused // 28 * 8 + n_2_h_2_w_2_co_2_fused // 32 * 4 + ax1)
                            v2 = T.axis.spatial(56, n_0_h_0_w_0_co_0_fused % 28 * 2 + n_2_h_2_w_2_co_2_fused % 32 // 16 + ax2)
                            v3 = T.axis.spatial(64, n_2_h_2_w_2_co_2_fused % 16 * 4 + ax3)
