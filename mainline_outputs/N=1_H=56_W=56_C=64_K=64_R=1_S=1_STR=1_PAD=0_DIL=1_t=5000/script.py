                ...
                for n_2_h_2_w_2_co_2_fused in T.thread_binding(128, thread="threadIdx.x"):
                    for n_3_init, h_3_init, w_3_init, co_3_init, n_4_init, h_4_init, w_4_init, co_4_init in T.grid(1, 4, 1, 2, 1, 1, 1, 1):
                        with T.block("conv2d_nhwc_init"):
                            ...
                            v_h = T.axis.spatial(56, h_4_init + n_0_h_0_w_0_co_0_fused // 14 * 4 + h_3_init)
                            v_w = T.axis.spatial(56, w_4_init + n_0_h_0_w_0_co_0_fused % 14 // 2 * 8 + n_2_h_2_w_2_co_2_fused // 16 + w_3_init)
                            v_co = T.axis.spatial(64, co_4_init + n_0_h_0_w_0_co_0_fused % 2 * 32 + n_2_h_2_w_2_co_2_fused % 16 * 2 + co_3_init)
                            T.reads()
                    ...
                    for rh_0, rw_0, rc_0 in T.grid(1, 1, 1):
                        for ax0_ax1_ax2_ax3_fused_0 in T.serial(16):
                            for ax0_ax1_ax2_ax3_fused_1 in T.thread_binding(128, thread="threadIdx.x"):
                                with T.block("PadInput_shared"):
                                    ...
                                    v1 = T.axis.spatial(56, n_0_h_0_w_0_co_0_fused // 14 * 4 + (ax0_ax1_ax2_ax3_fused_0 * 128 + ax0_ax1_ax2_ax3_fused_1) // 512)
                                    v2 = T.axis.spatial(56, n_0_h_0_w_0_co_0_fused % 14 // 2 * 8 + (ax0_ax1_ax2_ax3_fused_0 * 128 + ax0_ax1_ax2_ax3_fused_1) % 512 // 64)
                                    v3 = T.axis.spatial(64, (ax0_ax1_ax2_ax3_fused_0 * 128 + ax0_ax1_ax2_ax3_fused_1) % 64)
                            ...
                            for ax0_ax1_ax2_ax3_fused_1 in T.thread_binding(128, thread="threadIdx.x"):
                                for ax0_ax1_ax2_ax3_fused_2 in T.vectorized(4):
                                    with T.block("weight_shared"):
                                        ...
                                        v2 = T.axis.spatial(64, (ax0_ax1_ax2_ax3_fused_0 * 512 + ax0_ax1_ax2_ax3_fused_1 * 4 + ax0_ax1_ax2_ax3_fused_2) // 32)
                        ...
                        for rh_1, rw_1, rc_1, n_3, h_3, w_3, co_3, rh_2, rw_2, rc_2, n_4, h_4, w_4, co_4 in T.grid(1, 1, 16, 1, 4, 1, 2, 1, 1, 4, 1, 1, 1, 1):
                            with T.block("conv2d_nhwc_update"):
                                v_n = T.axis.spatial(1, n_3 + n_4)
                                v_h = T.axis.spatial(56, h_4 + n_0_h_0_w_0_co_0_fused // 14 * 4 + h_3)
                                v_w = T.axis.spatial(56, w_4 + n_0_h_0_w_0_co_0_fused % 14 // 2 * 8 + n_2_h_2_w_2_co_2_fused // 16 + w_3)
                                v_co = T.axis.spatial(64, co_4 + n_0_h_0_w_0_co_0_fused % 2 * 32 + n_2_h_2_w_2_co_2_fused % 16 * 2 + co_3)
                                v_rh = T.axis.reduce(1, rh_0 + rh_1 + rh_2)
                                v_rw = T.axis.reduce(1, rw_2 + rw_0 + rw_1)
                                v_rc = T.axis.reduce(64, rc_0 * 64 + rc_1 * 4 + rc_2)
                    ...
                    for ax0, ax1, ax2, ax3 in T.grid(1, 4, 1, 2):
                        with T.block("conv2d_nhwc_local"):
                            ...
                            v1 = T.axis.spatial(56, n_0_h_0_w_0_co_0_fused // 14 * 4 + ax1)
                            v2 = T.axis.spatial(56, n_0_h_0_w_0_co_0_fused % 14 // 2 * 8 + n_2_h_2_w_2_co_2_fused // 16 + ax2)
