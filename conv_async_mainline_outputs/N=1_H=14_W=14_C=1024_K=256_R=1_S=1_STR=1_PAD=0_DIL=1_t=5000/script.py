                ...
                for n_2_h_2_w_2_co_2_fused in T.thread_binding(64, thread="threadIdx.x"):
                    for n_3_init, h_3_init, w_3_init, co_3_init, n_4_init, h_4_init, w_4_init, co_4_init in T.grid(1, 1, 7, 1, 1, 1, 1, 1):
                        with T.block("conv2d_nhwc_init"):
                            ...
                            v_w = T.axis.spatial(14, w_4_init + n_2_h_2_w_2_co_2_fused % 32 // 16 * 7 + w_3_init)
                            v_co = T.axis.spatial(256, co_4_init + n_0_h_0_w_0_co_0_fused % 16 * 16 + n_2_h_2_w_2_co_2_fused % 16 + co_3_init)
                            T.reads()
                            ...
                            for ax0_ax1_ax2_ax3_fused_1 in T.thread_binding(64, thread="threadIdx.x"):
                                for ax0_ax1_ax2_ax3_fused_2 in T.vectorized(4):
                                    with T.block("PadInput_shared"):
                                        ...
                                        v1 = T.axis.spatial(14, n_0_h_0_w_0_co_0_fused // 16 * 2 + (ax0_ax1_ax2_ax3_fused_0 * 256 + ax0_ax1_ax2_ax3_fused_1 * 4 + ax0_ax1_ax2_ax3_fused_2) // 896)
                                        v2 = T.axis.spatial(14, (ax0_ax1_ax2_ax3_fused_0 * 256 + ax0_ax1_ax2_ax3_fused_1 * 4 + ax0_ax1_ax2_ax3_fused_2) % 896 // 64)
                                        v3 = T.axis.spatial(1024, rh_0_rw_0_rc_0_fused * 64 + (ax0_ax1_ax2_ax3_fused_0 * 256 + ax0_ax1_ax2_ax3_fused_1 * 4 + ax0_ax1_ax2_ax3_fused_2) % 64)
                            ...
                            for ax0_ax1_ax2_ax3_fused_1 in T.thread_binding(64, thread="threadIdx.x"):
                                for ax0_ax1_ax2_ax3_fused_2 in T.vectorized(4):
                                    with T.block("weight_shared"):
                                        ...
                                        v2 = T.axis.spatial(1024, rh_0_rw_0_rc_0_fused * 64 + (ax0_ax1_ax2_ax3_fused_0 * 256 + ax0_ax1_ax2_ax3_fused_1 * 4 + ax0_ax1_ax2_ax3_fused_2) // 16)
                                        v3 = T.axis.spatial(256, n_0_h_0_w_0_co_0_fused % 16 * 16 + (ax0_ax1_ax2_ax3_fused_0 * 256 + ax0_ax1_ax2_ax3_fused_1 * 4 + ax0_ax1_ax2_ax3_fused_2) % 16)
                                        T.reads(weight[v0, v1, v2, v3])
                                        T.writes(weight_shared[v0, v1, v2, v3])
                                        weight_shared[v0, v1, v2, v3] = weight[v0, v1, v2, v3]
                        for rh_1, rw_1, rc_1, n_3, h_3, w_3, co_3, rh_2, rw_2, rc_2, n_4, h_4, w_4, co_4 in T.grid(1, 1, 2, 1, 1, 7, 1, 1, 1, 32, 1, 1, 1, 1):
                            with T.block("conv2d_nhwc_update"):
                                ...
                                v_w = T.axis.spatial(14, w_4 + n_2_h_2_w_2_co_2_fused % 32 // 16 * 7 + w_3)
                                v_co = T.axis.spatial(256, co_4 + n_0_h_0_w_0_co_0_fused % 16 * 16 + n_2_h_2_w_2_co_2_fused % 16 + co_3)
                                v_rh = T.axis.reduce(1, rh_2 + rh_1)
                                v_rw = T.axis.reduce(1, rw_1 + rw_2)
                                v_rc = T.axis.reduce(1024, rh_0_rw_0_rc_0_fused * 64 + rc_1 * 32 + rc_2)
                    ...
                    for ax0, ax1, ax2, ax3 in T.grid(1, 1, 7, 1):
                        with T.block("conv2d_nhwc_local"):
                            ...
                            v2 = T.axis.spatial(14, n_2_h_2_w_2_co_2_fused % 32 // 16 * 7 + ax2)
