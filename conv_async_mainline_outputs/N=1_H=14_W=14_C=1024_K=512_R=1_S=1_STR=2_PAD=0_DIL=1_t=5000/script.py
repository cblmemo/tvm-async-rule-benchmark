...
class Module:
    ...
    def main(inputs: T.Buffer[(1, 14, 14, 1024), "float32"], weight: T.Buffer[(1, 1, 1024, 512), "float32"], conv2d_nhwc: T.Buffer[(1, 7, 7, 512), "float32"]):
        ...
        for n_0_h_0_w_0_co_0_fused in T.thread_binding(112, thread="blockIdx.x", annotations={"pragma_auto_unroll_max_step":16, "pragma_unroll_explicit":1}):
            for n_1_h_1_w_1_co_1_fused in T.thread_binding(1, thread="vthread.x"):
                for n_2_h_2_w_2_co_2_fused in T.thread_binding(32, thread="threadIdx.x"):
                    ...
                    for rh_0_rw_0_rc_0_fused in T.serial(16, annotations={"software_pipeline_async_stages":[0], "software_pipeline_order":[0, 1, 2], "software_pipeline_stage":[0, 0, 2]}):
                        for ax0_ax1_ax2_ax3_fused_0 in T.serial(26):
                            for ax0_ax1_ax2_ax3_fused_1 in T.thread_binding(32, thread="threadIdx.x"):
                                with T.block("PadInput_shared"):
                                    ...
                                    v1 = T.axis.spatial(14, (ax0_ax1_ax2_ax3_fused_0 * 32 + ax0_ax1_ax2_ax3_fused_1) // 64)
                                    v2 = T.axis.spatial(14, n_0_h_0_w_0_co_0_fused // 16 * 2)
                                    v3 = T.axis.spatial(1024, rh_0_rw_0_rc_0_fused * 64 + (ax0_ax1_ax2_ax3_fused_0 * 32 + ax0_ax1_ax2_ax3_fused_1) % 64)
                            ...
                            for ax0_ax1_ax2_ax3_fused_1 in T.thread_binding(32, thread="threadIdx.x"):
                                for ax0_ax1_ax2_ax3_fused_2 in T.vectorized(4):
                                    with T.block("weight_shared"):
                                        ...
                                        v2 = T.axis.spatial(1024, rh_0_rw_0_rc_0_fused * 64 + (ax0_ax1_ax2_ax3_fused_0 * 128 + ax0_ax1_ax2_ax3_fused_1 * 4 + ax0_ax1_ax2_ax3_fused_2) // 32)
                        ...
                        for rh_1, rw_1, rc_1, n_3, h_3, w_3, co_3, rh_2, rw_2, rc_2, n_4, h_4, w_4, co_4 in T.grid(1, 1, 1, 1, 7, 1, 1, 1, 1, 64, 1, 1, 1, 1):
                            with T.block("conv2d_nhwc_update"):
                                ...
                                v_rc = T.axis.reduce(1024, rh_0_rw_0_rc_0_fused * 64 + rc_1 * 64 + rc_2)
