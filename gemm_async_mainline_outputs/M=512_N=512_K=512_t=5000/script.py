            ...
            for i_1_j_1_fused in T.thread_binding(2, thread="vthread.x"):
                for i_2_j_2_fused in T.thread_binding(128, thread="threadIdx.x"):
                    for i_3_init, j_3_init, i_4_init, j_4_init in T.grid(8, 2, 1, 1):
                        with T.block("Y_init"):
                            v_i = T.axis.spatial(512, i_4_init + i_0_j_0_fused // 8 * 64 + i_2_j_2_fused // 16 * 8 + i_3_init)
                            v_j = T.axis.spatial(512, i_0_j_0_fused % 8 * 64 + i_1_j_1_fused * 32 + i_2_j_2_fused % 16 * 2 + j_3_init + j_4_init)
                            T.reads()
                            T.writes(Y_local[v_i, v_j])
                            T.block_attr({"meta_schedule.thread_extent_high_inclusive":1024, "meta_schedule.thread_extent_low_inclusive":32, "meta_schedule.tiling_structure":"SSSRRSRS"})
                            Y_local[v_i, v_j] = T.float32(0)
                    for k_0_fused in T.serial(16, annotations={"software_pipeline_async_stages":[0], "software_pipeline_order":[0, 1, 2], "software_pipeline_stage":[0, 0, 2]}):
                            ...
                            for ax0_ax1_fused_1 in T.thread_binding(128, thread="threadIdx.x"):
                                for ax0_ax1_fused_2 in T.vectorized(4):
                                    with T.block("A_shared"):
                                        ...
                                        v1 = T.axis.spatial(512, k_0_fused * 32 + (ax0_ax1_fused_0 * 512 + ax0_ax1_fused_1 * 4 + ax0_ax1_fused_2) % 32)
                                        T.reads(A[v0, v1])
                            ...
                            for ax0_ax1_fused_1 in T.thread_binding(128, thread="threadIdx.x"):
                                for ax0_ax1_fused_2 in T.vectorized(4):
                                    with T.block("B_shared"):
                                        v0 = T.axis.spatial(512, k_0_fused * 32 + (ax0_ax1_fused_0 * 512 + ax0_ax1_fused_1 * 4 + ax0_ax1_fused_2) // 64)
                                        v1 = T.axis.spatial(512, i_0_j_0_fused % 8 * 64 + (ax0_ax1_fused_0 * 512 + ax0_ax1_fused_1 * 4 + ax0_ax1_fused_2) % 64)
                                        T.reads(B[v0, v1])
                                        T.writes(B_shared[v0, v1])
                                        B_shared[v0, v1] = B[v0, v1]
                        for k_1, i_3, j_3, k_2, i_4, j_4 in T.grid(8, 8, 2, 4, 1, 1):
                            with T.block("Y_update"):
                                v_i = T.axis.spatial(512, i_4 + i_0_j_0_fused // 8 * 64 + i_2_j_2_fused // 16 * 8 + i_3)
                                v_j = T.axis.spatial(512, i_0_j_0_fused % 8 * 64 + i_1_j_1_fused * 32 + i_2_j_2_fused % 16 * 2 + j_3 + j_4)
                                v_k = T.axis.reduce(512, k_0_fused * 32 + k_1 * 4 + k_2)
                    ...
                    for ax0, ax1 in T.grid(8, 2):
                        with T.block("Y_local"):
                            ...
                            v1 = T.axis.spatial(512, i_0_j_0_fused % 8 * 64 + i_1_j_1_fused * 32 + i_2_j_2_fused % 16 * 2 + ax1)
