        ...
        for i_0_j_0_fused in T.thread_binding(128, thread="blockIdx.x", annotations={"pragma_auto_unroll_max_step":1024, "pragma_unroll_explicit":1}):
            for i_1_j_1_fused in T.thread_binding(1, thread="vthread.x"):
                for i_2_j_2_fused in T.thread_binding(128, thread="threadIdx.x"):
                    for i_3_init, j_3_init, i_4_init, j_4_init in T.grid(16, 2, 1, 2):
                        with T.block("Y_init"):
                            v_i = T.axis.spatial(1024, i_0_j_0_fused // 8 * 64 + i_2_j_2_fused // 32 * 16 + i_3_init + i_4_init)
                            v_j = T.axis.spatial(1024, i_0_j_0_fused % 8 * 128 + i_2_j_2_fused % 32 * 4 + j_3_init * 2 + j_4_init)
                            T.reads()
                            ...
                            for ax0_ax1_fused_1 in T.thread_binding(128, thread="threadIdx.x"):
                                for ax0_ax1_fused_2 in T.vectorized(4):
                                    with T.block("A_shared"):
                                        v0 = T.axis.spatial(1024, i_0_j_0_fused // 8 * 64 + (ax0_ax1_fused_0 * 512 + ax0_ax1_fused_1 * 4 + ax0_ax1_fused_2) // 32)
                                        v1 = T.axis.spatial(1024, k_0 * 32 + (ax0_ax1_fused_0 * 512 + ax0_ax1_fused_1 * 4 + ax0_ax1_fused_2) % 32)
                                        T.reads(A[v0, v1])
                            ...
                            for ax0_ax1_fused_1 in T.thread_binding(128, thread="threadIdx.x"):
                                for ax0_ax1_fused_2 in T.vectorized(2):
                                    with T.block("B_shared"):
                                        v0 = T.axis.spatial(1024, k_0 * 32 + (ax0_ax1_fused_0 * 256 + ax0_ax1_fused_1 * 2 + ax0_ax1_fused_2) // 128)
                                        v1 = T.axis.spatial(1024, i_0_j_0_fused % 8 * 128 + (ax0_ax1_fused_0 * 256 + ax0_ax1_fused_1 * 2 + ax0_ax1_fused_2) % 128)
                                        T.reads(B[v0, v1])
                                        T.writes(B_shared[v0, v1])
                                        B_shared[v0, v1] = B[v0, v1]
                        for k_1, i_3, j_3, k_2, i_4, j_4 in T.grid(8, 16, 2, 4, 1, 2):
                            with T.block("Y_update"):
                                v_i = T.axis.spatial(1024, i_0_j_0_fused // 8 * 64 + i_2_j_2_fused // 32 * 16 + i_3 + i_4)
                                v_j = T.axis.spatial(1024, i_0_j_0_fused % 8 * 128 + i_2_j_2_fused % 32 * 4 + j_3 * 2 + j_4)
                                v_k = T.axis.reduce(1024, k_0 * 32 + k_1 * 4 + k_2)
                    ...
                    for ax0, ax1 in T.grid(16, 4):
                        with T.block("Y_local"):
                            v0 = T.axis.spatial(1024, i_0_j_0_fused // 8 * 64 + i_2_j_2_fused // 32 * 16 + ax0)
